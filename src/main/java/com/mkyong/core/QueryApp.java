...
public class QueryApp {
	...
	public static void main(String[] args) {
		...
		address.setArea("Viveknagar");
		
		Address address2 = new Address();
		address2.setAddressType("office");
		address2.setArea("Koramangala");

		User user1 = new User("1001", "ant", 10, address);
		User user2 = new User("1002", "bird", 20, address2);
		User user3 = new User("1003", "cat", 30,address);
		User user4 = new User("1004", "dog", 40,address2);
		User user5 = new User("1005", "elephant",50,address);
		User user6 = new User("1006", "frog", 60,address2);
		users.add(user1);
		users.add(user2);
		users.add(user3);
		users.add(user4);
		...
		query11.addCriteria(Criteria.where("address.addressType").in("office").andOperator(Criteria.where("age").gt(40)));
		//query11.addCriteria(Criteria.where("name").in("dog"));
		List<User> userTest11 = mongoOperation.find(query11, User.class);
		System.out.println("query11 - " + query11.toString());
		for (User user : userTest11) {
			System.out.println("userTest11 - " + user);
		}
		...
		Address address1 = userTest1.getAddress();
		
		System.out.println("query1 - " + query1.toString());
		System.out.println("userTest1 - " + userTest1);
		System.out.println("address1 - " + address1.getAddressType());
